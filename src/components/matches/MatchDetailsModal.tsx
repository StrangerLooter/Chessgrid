        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-5">
          ...
          >
            ...
            <select
              ...
              className="px-2.5 py-1 rounded-lg text-xs font-bold text-[#e8c45a] focus:outline-none transition-all font-mono"
              style={{
            ...
            >
              {boards.map(b => (
                <option key={b.number} value={b.number}>
                  Board {b.number} {b.currentMatchId && b.currentMatchId !== match.id ? '(Occupied)' : ''}
                </option>
              ))}
            </select>
          </div>
          ...
          >
            <span className="text-xs font-semibold text-[#c8c0ae]/70">Time Control:</span>
            <span 
              className="text-xs font-bold px-2.5 py-1 rounded-lg font-mono text-white"
              style={{
                background: 'rgba(17, 17, 20, 0.9)',
                border: '1px solid rgba(201, 168, 76, 0.2)',
              }}
            >
              {match.timeControl.label} ({match.timeControl.initialMinutes}m + {match.timeControl.incrementSeconds}s)
            </span>
          </div>
        </div>

        {/* Dual Player Matchup & Clocks */}
          ...
          >
            <div className="flex items-center justify-between text-xs text-[#c8c0ae]/60 mb-2">
              <span className="font-bold flex items-center gap-1.5 text-white">
                <span className="w-3 h-3 rounded-full bg-white border border-slate-400 inline-block shadow-sm" />
                WHITE
              </span>
